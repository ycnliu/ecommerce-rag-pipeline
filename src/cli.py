...
@click.option(
    "--output-dir", "-o", default="models", help="Output directory for processed data"
)
...
@click.option(
    "--batch-size", "-b", default=32, help="Batch size for embedding generation"
)
    ...
    try:
        ...
        with tqdm(total=len(texts), desc="Embedding generation") as pbar:
            for i in range(0, len(texts), batch_size):
                ...
                batch_embeddings = embedding_service.batch_text_embeddings(
...
def search(ctx, query: str, k: int, rerank: bool, no_llm: bool, output_format: str):
    ...
    try:
        ...
        response = pipeline.query(
            text_query=query, k=k, rerank=rerank, generate_response=not no_llm
        )
...
def serve(ctx, host: str, port: int, reload: bool):
    ...
    try:
        ...
        uvicorn.run(
            app, host=host, port=port, reload=reload, log_level=config.log_level.lower()
        )
...
@click.option(
    "--model-name", default="openai/clip-vit-base-patch32", help="Base CLIP model"
)
@click.pass_context
def train_clip(
    ctx,
    ...
    try:
        ...
        if use_wandb:
            try:
                ...
                click.echo(
                    "⚠️ Weights & Biases not available, continuing without logging"
                )
                use_wandb = False
        ...
        fine_tuner = CLIPFineTuner(
            model_name=model_name, device=config.device or "auto"
        )
...
@click.option(
    "--output-dir", default="models/fusion", help="Output directory for trained model"
)
@click.option("--use-wandb", is_flag=True, help="Use Weights & Biases logging")
@click.option(
    "--clip-model", default="openai/clip-vit-base-patch32", help="CLIP model to use"
)
@click.option(
    "--sentence-model",
...
def push_to_hub(
    ...
    try:
        ...
        try:
            ...
            create_repo(
                repo_id=repo_name, private=private, exist_ok=True, token=hf_token
            )
...
def _initialize_rag_pipeline(config: Config) -> RAGPipeline:
    ...
    if not config.validate_llm_config():
        raise click.ClickException(
            "LLM configuration is invalid. Check your API token."
        )
